...
internal class ApiRequestMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext httpContext)
    {
        ...
        var actionResponseList = new List<ApiResponse>();
        ...
        foreach (var request in body.AsArray())
        {
            ...
            try
            {
                ...
                actionResponseList.Add(new ApiResponse(val));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Batch API error");
                actionResponseList.Add(new ApiResponse(new ErrorResponse(500, ex.Message), 600));
            }
        }

        var responseData = new ServerResponse<IEnumerable<ApiResponse>>(actionResponseList, 200);
    ...
    private async Task<JsonNode> GetDeserializedBody(HttpContext httpContext)
    {
        ...
        if (httpContext.Request.ContentType?.Contains("multipart/form-data") == true)
        {
            ...
            return JsonNode.Parse(bodySplit[3]);
        }
